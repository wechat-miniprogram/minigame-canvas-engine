...
export default class ScrollView extends View {
    ...
    get scrollHeight() {
        ...
        const last = this.children[ids[ids.length - 1]];
    ...
    calPageData() {
        ...
        for ( let i = 0; i < this.pageCount; i++ ) {
            const cache = this.root.canvasPool.get(i);
            if ( cache ) {
    ...
    renderChildren(tree) {
        ...
        Object.keys(children).forEach( id => {
            const child   = children[id];
            const originY   = child.layoutBox.originalAbsoluteY;
            const pageIndex = Math.floor(originY / height);
            ...
            if ( originY + child.layoutBox.height > height * nextPage ) {
                const tmpBox = Object.assign({}, child.layoutBox);
    ...
    insertElements(pageIndex) {
        ...
        const ctx    = can.getContext('2d',{alpha : true});
